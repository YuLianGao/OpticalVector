    ...
    while(!doStop)
    {
        ...
        for(int i = processingBuffer.size(); i < processingBufferLength && getCurrentFramenumber() < lengthInFrames; i++) {
            ...
            if(cap.read(grabbedFrame)) {
                ...
                if(imgProcFlags.rotateOn)
                {
                     currentFrame = rotate(currentFrame,imgProcSettings.rotationValue );

                     qDebug() << " PlayerThread.cpp imgProcSettings.rotationValue =" << imgProcSettings.rotationValue;
                }
                // Fill fuffer
...
void PlayerThread::updateImageProcessingFlags(struct ImageProcessingFlags imgProcessingFlags)
{
    ...
    this->imgProcFlags.rotateOn = imgProcessingFlags.rotateOn;
    locker1.unlock();
    locker2.unlock();
...
void PlayerThread::updateImageProcessingSettings(struct ImageProcessingSettings imgProcessingSettings)
{
    ...
    this->imgProcSettings.rotationValue = imgProcessingSettings.rotationValue;
...
Mat PlayerThread::rotate(Mat src, double angle)
{
    Mat dst;
